...
# Check if scene exists. If not, insert it later. Check again later because other rp may have been registered in the meantime
	sql = "SELECT * FROM Scene WHERE SceneId = '{0}'".format(activity['sceneid'])
	sresult = db_fetchone(sql)
	logging.warning('publishOneMS3 {} scene fetchone {} {}'.format(activity['sceneid'],sql,sresult))
# If scene is marked Deleted by Quality Control, no need to go on
	if sresult is not None and sresult['Deleted'] == 1:
		activity['message'] = 'Deleted scene'
		ret = db_end(activity)
		if ret is None: 
			activity['status'] = 'ERROR'
			activity['message'] = 'Connection error'
...
# Check if products have already been registered. If yes, no need to register
	sql = "SELECT * FROM Asset WHERE SceneId = '{0}' AND Dataset = '{1}'".format(activity['sceneid'],activity['dataset'])
	presult = db_fetchone(sql)
	logging.warning('publishOneMS3 {} asset fetchone {} {}'.format(activity['sceneid'],sql,presult))
	if presult is not None:
		assets = json.loads(presult['Assets'])
		nassets = len(assets)
		logging.warning('publishOneMS3 {} Products {}/{}'.format(activity['sceneid'],nassets,len(activity['files'])))
		if nassets == len(activity['files']) and sresult is not None :
			activity['message'] = 'Already done'
			ret = db_end(activity)
			if ret is None: 
				activity['status'] = 'ERROR'
...
# Start filling metadata structure for tables Scene and Asset in Catalogo database
	result = {'Scene':{},'Asset':{}}
	result['Asset']['Dataset'] = activity['dataset']
	result['Asset']['SceneId'] = activity['sceneid']
	result['Asset']['Assets'] = []
...
# Check if scene exists in SceneDataset. If not create it.
	sql = "SELECT * FROM SceneDataset WHERE SceneId = '{0}' AND Dataset = '{1}'".format(activity['sceneid'],activity['dataset'])
...
# Inserting data into Scene table
	sqlScene = "SELECT * FROM Scene WHERE SceneId = '{0}'".format(activity['sceneid'])
	sresult = db_fetchone(sqlScene)
	logging.warning('publishOneMS3 {} fetchone {} {}'.format(activity['sceneid'],sqlScene,sresult))
	if sresult is None:
		params = ''
		values = ''
...
# Inserting data into Asset table
	for band in config['publish'][activity['rp']][activity['inst']]:
		...
		result['Asset']['Assets'].append({'band':band,'href':file})

	params = ''
	values = ''
	for key in ['Dataset','SceneId']:
		params += key+','
		values += "'{0}',".format(result['Asset'][key])
		
	params += 'Assets'
	values += "'{0}'".format(json.dumps(result['Asset']['Assets']))
	sqlAsset = "INSERT INTO Asset ({0}) VALUES({1})".format(params,values)
